/*!/wp-content/themes/everfx_3.0/assets/ScrollMagic/animation.gsap.min.js*/ /*!ScrollMagic v2.0.8 | (c) 2020 Jan Paepke (@janpaepke) | license & info: http://scrollmagic.io*/ !(function (
  e,
  n
) {
  var r;
...
/*!/wp-content/themes/everfx_3.0/assets/ScrollMagic/TweenMax.min.js*/ /*!
 * VERSION: 2.1.2
 * DATE: 2019-03-01
...
/*!/wp-content/themes/everfx_3.0/assets/jq/jquery.validate.min.js*/ /*! jQuery Validation Plugin - v1.19.0 - 11/28/2018
 * https://jqueryvalidation.org/
 * Copyright (c) 2018 J̦rn Zaefferer; Licensed MIT */ !(function (a) {
  "function" == typeof define && define.amd
    ? define(["jquery"], a)
...
/*!/wp-content/cache/asset-cleanup/js/item/everfx-login-modal-vd6feb82964bbfc55c8b14edd9ca61e904d4f6d83.js*/ /*!/wp-content/themes/everfx_3.0/js/dist/login.js*/ ((
  $
) => {
  $(document).ready(() => {
    $(".forgot-password").on("click", () => {
      ...
      $('input[name="forgotPasswordEmail"]').val(
        $('input[name="loginEmail"]').val()
      );
      $('input[name="forgotPasswordEmail"]').parent().addClass("focused");
    });
    $(".modal-dialog_login .close, .return-to-login").on("click", () => {
      $(".login-form-container").show();
      $(".forgot-password-form-container").hide();
      $(".forgot-password-success-message").hide();
    });
    $(".modal-dialog_login .close").on("click", () => {
      $("#forgotPasswordForm")[0].reset();
      $("#loginForm")[0].reset();
      $("#forgotPasswordForm .form-group, #loginForm .form-group").removeClass(
        "focused"
      );
      $(
        "#forgotPasswordForm .form-control, #loginForm .form-control"
      ).removeClass("valid success error");
      $("#forgotPasswordForm .error-message, #loginForm .error-message").hide();
      $("#forgotPasswordForm .api-error, #loginForm .api-error").hide();
    });
    ValidateSettings();
    RegisterHandlers();
  });
  $.extend($.validator.messages, {
    ...
    number: "Please enter a digits.",
    digits: "Please enter a digits.",
    ...
    rangelength: $.validator.format(
      "Please enter a value between {0} to {1} characters."
    ),
    range: $.validator.format("Please enter a number between {0} to {1}."),
  ...
  var SetValidateEvent = function (fm) {
    $(fm).validate({
      errorPlacement: function errorPlacement(error, element) {
        var container = $(element).parent();
        $(container).find("span.error-message").html($(error).html());
        $(container).find("span.error-message").show();
        element.removeClass("success");
      },
      success: function success(error) {
        var element = $("form").find(
          'input[name="' + $(error).attr("for") + '"]'
        );
        var container = $(element).parent();
        $(container).find("span.error-message").html("");
        $(container).find("span.error-message").hide();
  ...
  var sendRegistrationRequest = function () {
    $("#loginForm .api-error").hide();
    var button = $('#loginForm button[type="submit"]');
    var buttonText = $(button).html();
    $(button).html('<div class="loader"></div>').attr("disabled", "disabled");
    var fieldsArray = {
      email: $('input[name="loginEmail"]').val(),
      password: $('input[name="loginUserPassword"]').val(),
    };
    $.ajax({
      url: "/wp-content/themes/bold-news/api/falcon/login.php",
      type: "POST",
      data: $.param(fieldsArray),
      dataType: "text",
      success: function success(data) {
        var data = JSON.parse(data);
        $(button).html(buttonText).removeAttr("disabled");
        if (data.error) {
          $("#loginForm .api-error").show();
        } else {
          $("#loginForm")[0].reset();
...
/*!/wp-content/cache/asset-cleanup/js/item/everfx-reset-password-modal-v489d1193ae3c0cbda8de9c85132495790f083ade.js*/ /*!/wp-content/themes/everfx_3.0/js/dist/reset-password.js*/ ((
  $
) => {
  $(document).ready(() => {
    ValidateSettings();
  ...
  var sendRegistrationRequest = function () {
    $("#forgotPasswordForm .api-error").hide();
    var button = $('#forgotPasswordForm button[type="submit"]');
    var buttonText = $(button).html();
    var userEmail = $('input[name="forgotPasswordEmail"]').val();
    var mailService = userEmail.split("@");
    $(button).html('<div class="loader"></div>').attr("disabled", "disabled");
    var fieldsArray = { email: userEmail };
    $.ajax({
      url: "/wp-content/themes/bold-news/api/falcon/reset-password.php",
      type: "POST",
      data: $.param(fieldsArray),
      dataType: "text",
      success: function success(data) {
        ...
        $(button).html(buttonText).removeAttr("disabled");
        if (data.error) {
          $("#forgotPasswordForm .api-error").show();
        } else {
          $(".forgot-password-form-container").hide();
          $(".reset-password-user-email")
            .attr("href", "https://" + mailService[1])
            .text(userEmail);
...
/*!/wp-content/cache/asset-cleanup/js/item/everfx-cookies-v07727aebdf68f8558d4f149ce4fb661e1154fdfa.js*/ /*!/wp-content/themes/everfx_3.0/js/dist/registration_v_3.0.0/cookies.js*/ var setCookie =
...
var getUrlParamFromCookies = function (cname, pname) {
  var name = pname + "=";
  var getQuery = getCookie(cname);
  if (getQuery != "") {
    var getParameters = getQuery.split("&");
...
/*!/wp-content/cache/asset-cleanup/js/item/everfx-user-entry-vcb75c4d2b27f469d7e1fcfa6188e55de107bf7f5.js*/ /*!/wp-content/themes/everfx_3.0/js/dist/registration_v_3.0.0/user-entry.js*/ (function (
  $
) {
  $(document).ready(function () {
    var getQuery = window.location.href.split("?");
    var userStartUrl = window.location.origin + window.location.pathname;
    if (getCookie("user_start_url") == "") {
      setCookie("user_start_url", userStartUrl, 30);
    }
    if (getQuery.length == 2) {
      if (getCookie("regParams") == "") {
        var _params = getQuery[1];
        setCookie("regParams", _params, 30);
      }
    } else {
      if (getCookie("regParams") == "") {
        setCookie("regParams", "", 30);
      }
    }
    var lockedLicense = getUrlParamFromCookies("regParams", "fxbl");
    if (lockedLicense !== "" && lockedLicense !== undefined) {
      setCookie("license", lockedLicense, 30);
    } else {
      var licenseSelected = getCookie("license");
      if (licenseSelected == undefined || licenseSelected == "") {
        setCookie("license", "fsaeverfx", 30);
      }
    }
    push_params();
  });
  function push_params() {
    const elem = $(".push-params");
    const params = getCookie("regParams");
    const push = function (e) {
      e.preventDefault();
      const href = $(this).attr("href");
      const newUrl = href + "?" + params;
      if (params != "") {
...
/*!/wp-content/cache/asset-cleanup/js/item/everfx-licenses-v58868a9c5c70ab776522c5f6349d0b5cc39b5682.js*/ /*!/wp-content/themes/everfx_3.0/js/dist/registration_v_3.0.0/licenses.js*/ (function (
  $
) {
  $(document).ready(function () {
    var licenseSelected = getCookie("license");
    var addActiveClass = function (license) {
      $("#regionModal .regionModalCell").removeClass("active");
      $(".license-popup-triger").css({ "font-weight": 100 });
      $("." + license + ".license-popup-triger").css({ "font-weight": 800 });
      $("#regionModal .regionModalCell a")
        .removeClass("btnGreen btnWhite")
        .addClass("btnWhite");
      $("#regionModal .regionModalCell." + license).addClass("active");
      $("#regionModal .regionModalCell a." + license)
        .removeClass("btnGreen btnWhite")
        .addClass("btnGreen");
      $('input[name="to"]').val(
        license === "everfxglobal"
          ? "support@everfxglobal.com"
          : license === "everfx"
          ? "support@everfx.eu"
          : "support@everfxint.com"
      );
      $(".package").addClass("d-none");
      $(".package-" + license).removeClass("d-none");
      $(".pm-license").addClass("d-none");
      $(".license-" + license).removeClass("d-none");
      $(".live-fsaeverfx").show();
      if (license == "everfxglobal") {
        $(".license-container").text("CIMA");
      } else if (license == "fsaeverfx") {
        $(".license-container").text("FSA");
      } else {
        $(".license-container").text("CYSEC");
      }
    };
    addActiveClass(licenseSelected || "fsaeverfx");
    setTimeout(function () {
      $(".live-fsaeverfx").show();
    }, 1000);
    $("a.everfxglobal:not(.license-popup-triger)").on("click", function () {
      addActiveClass("everfxglobal");
    });
    $("a.fsaeverfx:not(.license-popup-triger)").on("click", function () {
      addActiveClass("fsaeverfx");
    });
  });
})(jQuery);
/*!/wp-content/themes/everfx_3.0/js/dist/home.js*/ $(document).ready(
  function () {
    ...
    $("#homeSlider").slick({
      ...
      $("#homeSlider")[0].slick.slickGoTo(parseInt(1));
    for (
      var h1Text = $('.slick-slide[data-slick-index="1"]')
          .find(".home__section1__slide")
          .attr("data-h1"),
        newstring = "",
        i = 0;
      i < h1Text.length;
      ...
      newstring = newstring + "<span>" + h1Text.charAt(i) + "</span>";
    $("h1 i").html(newstring),
    ...
    var currentSlide = $("#homeSlider").slick("slickCurrentSlide");
    $('.slick-slide[data-slick-index="' + (currentSlide - 1) + '"]').addClass(
      "leaving"
    ),
      $('.slick-slide[data-slick-index="' + currentSlide + '"]').addClass(
        "active"
      ),
      $('.slick-slide[data-slick-index="' + (currentSlide + 1) + '"]').addClass(
        "coming"
          ...
          if (currentSlide != nextSlide)
            if (currentSlide < nextSlide) {
              ...
              $("h1 i").html(newstring),
                ...
                $(".slick-slide").removeClass("leaving"),
                $(".slick-slide").removeClass("active"),
                $(".slick-slide").removeClass("coming"),
                $(
                ...
                $(
                  '.slick-slide[data-slick-index="' + (currentSlide + 2) + '"]'
                ).addClass("coming");
            } else {
              for (
                h1Text = $(
      ...
      $("#platformSocial").hover(
        function () {
          $(".iconSystemIos").css("opacity", "0.2"),
            $(".iconSystemAndroid").css("opacity", "0.2");
        },
        function () {
          $(".iconSystemIos").css("opacity", "1"),
            $(".iconSystemAndroid").css("opacity", "1");
        }
      ),
      $("#platformMobile").hover(
        function () {
          $(".iconSystemWindows").css("opacity", "0.2"),
            $(".iconSystemMac").css("opacity", "0.2");
        },
        function () {
          $(".iconSystemWindows").css("opacity", "1"),
            $(".iconSystemMac").css("opacity", "1");
        }
      );
    ...
    new ScrollMagic.Scene({
      ...
      .addTo(scrollMagicController)
      .reverse(!1);
    var tweenScene2Scrolling = new TimelineMax();
    tweenScene2Scrolling.add([
      TweenMax.fromTo(" .section2ScrollingA", 1, { y: -30 }, { y: 40 }),
      TweenMax.fromTo(" .section2ScrollingB", 1, { y: 30 }, { y: -40 }),
      TweenMax.fromTo(
        " .section2ScrollingC",
        1,
        { y: -30, opacity: 1 },
    ...
    new ScrollMagic.Scene({
      ...
      .addTo(scrollMagicController)
      .reverse(!1);
    var tweenScene3Scrolling = new TimelineMax();
    tweenScene3Scrolling.add([]);
    new ScrollMagic.Scene({
      triggerElement: ".home__section3",
      triggerHook: "onEnter",
      offset: 0,
    ...
    new ScrollMagic.Scene({
      ...
      .addTo(scrollMagicController)
      .reverse(!1);
    var tweenScene4Scrolling = new TimelineMax();
    tweenScene4Scrolling.add([
      TweenMax.fromTo(" .section4ScrollingA", 1, { y: -30 }, { y: 40 }),
      TweenMax.fromTo(" .section4ScrollingB", 1, { y: 30 }, { y: -40 }),
      TweenMax.fromTo(" .section4ScrollingC", 1, { x: 0 }, { x: -40 }),
    ]);
    new ScrollMagic.Scene({
      triggerElement: ".home__section4",
      triggerHook: "onEnter",
    ...
    new ScrollMagic.Scene({
      ...
      .addTo(scrollMagicController)
      .reverse(!1);
    var tweenScene5Scrolling = new TimelineMax();
    tweenScene5Scrolling.add([
      TweenMax.fromTo("#iconClients1", 1, { x: 0, y: -30 }, { x: 0, y: 40 }),
      TweenMax.fromTo("#iconClients2", 1, { x: 0, y: 30 }, { x: 0, y: -40 }),
      TweenMax.fromTo("#iconClients3", 1, { x: 10, y: 10 }, { x: 20, y: 40 }),
      TweenMax.fromTo("#iconClients4", 1, { x: 0, y: -20 }, { x: 0, y: 40 }),
      TweenMax.fromTo("#iconClients5", 1, { x: -20, y: -5 }, { x: 0, y: 40 }),
      TweenMax.fromTo("#iconClients6", 1, { x: 10, y: 0 }, { x: 50, y: 10 }),
      TweenMax.fromTo("#iconClients7", 1, { x: 5, y: 25 }, { x: -10, y: 15 }),
      TweenMax.fromTo("#iconClients8", 1, { x: 0, y: -20 }, { x: 40, y: -40 }),
      TweenMax.fromTo("#iconClients9", 1, { x: -15, y: 10 }, { x: 0, y: 10 }),
      TweenMax.fromTo("#iconClients10", 1, { x: 30, y: -10 }, { x: 0, y: -20 }),
    ]);
    new ScrollMagic.Scene({
      triggerElement: ".home__section5__clients",
      triggerHook: "onEnter",
      offset: 0,
      duration: 1500,
    })
      .setTween(tweenScene5Scrolling)
      .addTo(scrollMagicController)
      .reverse(!0);
    var tweenScene5Logos = new TimelineMax();
    tweenScene5Logos.add([
      TweenMax.staggerFromTo(
        " .home__section5__logos span",
        1,
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0 },
        0.1
    ...
    new ScrollMagic.Scene({
      triggerElement: ".home__section5__logos",
      triggerHook: "onEnter",
      offset: 0,
    })
      .setTween(tweenScene5Logos)
      .addTo(scrollMagicController)
      .reverse(!1);
    ...
    tweenScene6.add([
      TweenMax.staggerFromTo(
        ...
        2,
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0, ease: Power2.easeOut, delay: 0.5 },
        0.2
      ),
      TweenMax.staggerFromTo(
        " .section6AnimB",
    ...
    new ScrollMagic.Scene({
      ...
      .addTo(scrollMagicController)
      .reverse(!1);
    var tweenScene6b = new TimelineMax();
    tweenScene6b.add([
      TweenMax.staggerFromTo(
        " .section6bAnim",
        2,
    ...
    new ScrollMagic.Scene({
      triggerElement: ".home__section6__openAccount",
      triggerHook: "onEnter",
      offset: 0,
    })
      .setTween(tweenScene6b)
      .addTo(scrollMagicController)
      .reverse(!1);
  }
);
/*!/wp-content/themes/everfx_3.0/js/dist/main.js*/ var everfxGlobal = {
  ajaxurl: "https://everfx.com/wp-admin/admin-ajax.php",
};
function checkSVGs() {
  Modernizr.svg &&
  ...
  $(document).ready(function () {
    $("#regionModal").on("hidden.bs.modal", function (e) {
      ...
      $(".button--animated").each(function () {
        var string = $(this).attr("data-text"),
          icon = $(this).attr("data-icon");
        "" != icon && (icon = '<i class="' + icon + '"></i>');
        for (var newstring = "", i = 0; i < string.length; i++)
          (newstring = newstring + "<span>" + string.charAt(i) + "</span>"),
            $(this).html(newstring + icon);
      }),
      $(".stickyWarning").click(function (e) {
        e.preventDefault(),
          $(".stickyWarning i").toggleClass("fa-angle-down"),
          $(".stickyWarning i").toggleClass("fa-angle-up"),
          $(".stickyToggle").slideToggle(),
          $(".stickyTitle").slideToggle();
      }),
      ...
      jQuery(".hamburger").click(function () {
        jQuery(this).toggleClass("is-active"),
          jQuery("html").toggleClass("mobileMenuOpen"),
          jQuery(".mobileMenu").hasClass("active")
            ? ((modalOpen = !1),
              ...
              TweenMax.fromTo(
                ...
                1,
                { opacity: 1, y: 0 },
                { opacity: 0, y: 0, ease: Power1.easeIn, delay: 0.8 }
              ),
              TweenMax.staggerFromTo(
                ".menuItems",
                1,
                { opacity: 1, y: 0 },
                { opacity: 0, y: 0, ease: Power1.easeIn },
                0.1
              ),
              TweenMax.staggerFromTo(
                ".headerMobileAnim",
            ...
            : ((modalOpen = !0),
              ...
              TweenMax.fromTo(
                ...
                1,
                { opacity: 0, y: 0 },
                { opacity: 1, y: 0, ease: Power1.easeIn, delay: 0.4 }
              ),
              TweenMax.staggerFromTo(
                ".headerMobileAnim",
                1,
              ...
              TweenMax.staggerFromTo(
                ...
                1,
                { opacity: 0, y: 40 },
                { opacity: 1, y: 0, ease: Power1.easeOut, delay: 0.4 },
                0.1
              ));
      });
    var scrollMagicController = new ScrollMagic.Controller(),
      tweenHeader = new TimelineMax();
    tweenHeader.add([
      TweenMax.fromTo(
      ...
      TweenMax.staggerFromTo(
        ...
        1,
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0, ease: Power2.easeOut, clearProps: "all" },
        0.05
      ),
    ]);
    new ScrollMagic.Scene({
    ...
    tweenFooter.add([
      TweenMax.staggerFromTo(
        ...
        1,
        { opacity: 0 },
        { opacity: 1, ease: Power2.easeOut, clearProps: "all" },
        0.2
      ),
    ]);
    new ScrollMagic.Scene({
...
var controller = new ScrollMagic.Controller({
  ...
  pin = new ScrollMagic.Scene({ triggerElement: ".header__nav" })
    .setPin(".header__nav", { pushFollowers: !1 })
    .addTo(controller);
...
pin.on("enter", function (event) {
  $(".header__nav").addClass("sticky");
}),
  pin.on("leave", function (event) {
